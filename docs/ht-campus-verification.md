...
| Student/calendar/classroom/course views and routing | 56 tests passed | Escaping, URL handling, module locks, calendar source preservation, classroom/replay authorization, draft identities, role controls, unique session routes, own-message previews, unavailable-member history, failed-read recovery, grade calculations, extended deadlines and explicit stale-review recovery |
...
Database, store, view and all application browser suites were rerun for this coursework milestone. Edge and legacy-harness results above are retained from the preceding classroom verification; coursework did not change those implementations.
