...
The frontend includes an optional first-visit invitation, role-specific guides at `/ht/hub/welcome/`, contextual directions, a searchable site map and permanent Guide links. No additional database migration is needed. Progress is browser-local and isolated by actual member identity, role and demo/live mode; it records places explored rather than academic completion. See [onboarding behavior and rehearsal](ht-onboarding-guide.md).
