...
### 15.1 Spike results — all pre-build gates closed 2026-09-11

Run against the production app `0f38f396-7ab8-41ed-bc08-8cf48fa695c7` with the five real presets; every test meeting retired (`PATCH status INACTIVE`) afterwards.

- **1(c) `<rtk-meeting>` props — CLOSED.** Nothing is required in practice: every prop has a default. Observed attributes: `meeting, iconPack, t, config, overrides, load-config-from-preset, apply-design-system, mode, leave-on-unmount, show-setup-screen, size, grid-layout`. Defaults read off a live element: `mode='fixed'`, `size='lg'`, `gridLayout='row'`, `applyDesignSystem=true`, `loadConfigFromPreset=true`, `leaveOnUnmount=false`. **v1 sets only `meeting` and `showSetupScreen`** and leaves the rest at their defaults; `mode='fill'` is applied only if the room does not fill its container in the first rehearsal.
- **1(g) hiding the stock recording toggle — CLOSED.** `rtk-recording-toggle` appears at exactly one path in `defaultConfig`: `root['rtk-more-toggle'].activeMoreMenu[5][0]`. Remove that one entry (`structuredClone(defaultConfig)`, splice index 5, assign to `el.config`) and the More menu loses it. Keep `rtk-recording-indicator`, which sits at `root['div#header-left'][1]` and `root['div#header-left.sm'][1]` — that is the REC pill the host should still see. Confirmed live: with the `opil-host` preset the rendered controls are `rtk-settings-toggle, rtk-screen-share-toggle, rtk-mic-toggle, rtk-camera-toggle, rtk-leave-button, rtk-more-toggle, rtk-chat-toggle, rtk-polls-toggle, rtk-participants-toggle, rtk-plugins-toggle, rtk-ai-toggle, rtk-stage-toggle, rtk-fullscreen-toggle` — the recording toggle renders lazily inside More.
- **2 participant token lifetime — CLOSED.** TTL is **2,400 hours (100 days)**; claims are `exp, iat, meetingId, orgId, participantId, presetId` — no PII, no email. `ea-rtk-join` still re-issues on every join (cheap, and it keeps preset changes honest), but nothing expires mid-class.
- **4 judge no-media preset inside a `GROUP_CALL` class — CLOSED, works.** A participant on `opil-judge` joined the same meeting as an `opil-host` and rendered **no mic, no camera, no share** controls at all — only Full Screen, Settings, Leave, More, Chat, Polls, Participants. Permissions read back exactly as written: `canProduceVideo/Audio/Screenshare = NOT_ALLOWED`, `chatPublic.canSend = true`, files `false`, `polls.canCreate = false`, `canVote = true`, mute-all/kick/record `false`. Mixed presets in one meeting behave as documented.
- **5 meeting size cap — no API-side cap found.** 120 participants were added to one meeting with no error (`POST …/participants` ×120, zero failures). That is participant *records*, not concurrent media: the concurrent-publisher ceiling is still a rehearsal measurement, and Web Core 2.0.2's release note ("grids with up to 100 participants per page") is the only published number. Both rooms are far below it (27 OPIL, ~50 Academy).
- **19 local Supabase stack — CLOSED, and it fails: staging must be a second hosted project.** This Mac has no Docker (`docker` not on PATH, no `Docker.app`, no colima) and no local Postgres, so `supabase start` / `db reset` cannot run here at all. §11 step 3 and §14.1 therefore use **a second hosted Supabase project** as staging; `supabase/config.toml` and `seed.sql` are not created. This needs Nelson's nod (a free-tier project) — §17 item 10.

**Preset creation found two undocumented required fields** (the API rejected all five bodies until they were added, 2026-09-11): `permissions.plugins.config` (object, `{}` is fine) and `ui.design_tokens.spacing_base` (number, `4`). The five committed bodies in `scripts/rtk-presets/` carry both, plus the full Academy design tokens (gold brand ramp around `#fdc921`, navy background ramp `#04123a`–`#22345f`, `text_on_brand: #04123a`, `video_bg: #0a1733`) — so the room is branded by the preset itself, and `provideRtkDesignSystem` only has to match it. Verified visually: navy stage, gold avatars, on both host and judge.
...
10. **Staging project.** This Mac cannot run the local Supabase stack (no Docker, no Postgres — §15.1 spike 19), so testing needs a **second hosted Supabase project** (free tier) with the same migrations and two seeded test users. Create it, or test against production with throwaway rows?
