...
--   Early Bird          $75   public, from now → Sat Oct 10, 11:59 PM CT
--   General             $90   public, Sun Oct 11 → Sat Oct 17, 6 PM CT
--   Build With Me (In Person)  $200 x15, public, from now → Sat Oct 17, noon CT
...
begin
  ...
  update public.ea_events
     set starts_at = timestamptz '2026-10-17 19:00 America/Chicago',
         ends_at   = timestamptz '2026-10-17 21:00 America/Chicago',
         status    = 'on_sale'
   where id = v_agent;
  ...
  insert into public.ea_ticket_tiers (event_id, name, description, price_cents, qty, sales_start, sales_end, access, sort) values
    (v_agent, 'AI 101 class price', 'For AI 101 sign-ups and the waitlist, through your personal link. 48 hours only.',
       6500, 500, timestamptz '2026-10-03 21:00 America/Chicago', timestamptz '2026-10-05 21:00 America/Chicago', 'waitlist', 1),
    (v_agent, 'Early Bird', 'Online, live on the night. You build along with me and leave with a working agent and the playbook.',
       7500, 500, now(), timestamptz '2026-10-10 23:59 America/Chicago', 'public', 2),
    (v_agent, 'General', 'Online, live on the night. You build along with me and leave with a working agent and the playbook.',
       9000, 500, timestamptz '2026-10-11 00:00 America/Chicago', timestamptz '2026-10-17 18:00 America/Chicago', 'public', 3),
...
begin
  ...
  select id into v_ai from public.ea_events
   where workshop_slug = 'ai101' and starts_at = timestamptz '2026-10-03 19:00 America/Chicago';
  if v_ai is null then
    ...
    values ('ai101', 'AI 101: Learn to Talk to AI',
            ...
            timestamptz '2026-10-03 19:00 America/Chicago', timestamptz '2026-10-03 19:45 America/Chicago',
    ...
    update public.ea_events set join_url = v_room, status = 'on_sale' where id = v_ai;
  end if;
  ...
  if not exists (select 1 from public.ea_ticket_tiers where event_id = v_ai and price_cents = 0 and status = 'active') then
    ...
    values (v_ai, 'Free seat', 'Live online, about 45 minutes.', 0, 500, now(),
            timestamptz '2026-10-03 19:30 America/Chicago', 'public', 1);
  end if;
end $$;
