...
const IDS = {classmate:'10000000-0000-4000-8000-000000000005',career:'20000000-0000-4000-8000-000000000002',instructor:'10000000-0000-4000-8000-000000000008',cohort:'a0000000-0000-4000-8000-000000000001',otherCohort:'a0000000-0000-4000-8000-000000000002',student:'10000000-0000-4000-8000-000000000001',staff:'10000000-0000-4000-8000-000000000002',leadership:'10000000-0000-4000-8000-000000000003',other:'10000000-0000-4000-8000-000000000004',course:'20000000-0000-4000-8000-000000000001',event:'30000000-0000-4000-8000-000000000001'};
const clone = value => JSON.parse(JSON.stringify(value));
/** Bump when the built-in sample records change, so an older browser snapshot resets to them. */
const FIXTURES_REV = 4;
...
function upgradeAcademics(s,now){
 ...
 if(s.assignments.some(a=>a.id===seeds[0].id)&&!s.assignment_attempts.some(a=>a.assignment_id===seeds[0].id))s.assignment_attempts.push({id:'e0000000-0000-4000-8000-000000000001',assignment_id:seeds[0].id,user_id:IDS.classmate,attempt_no:1,body:'Project: a study plan helper for first-year students. Audience: students taking two lab sciences. Prompt: "Draft a five-day study plan for two biology chapters, 45 minutes a day, with retrieval practice. Ask me what is missing before you plan." I checked the dates against the syllabus and replaced one topic we have not covered. Limitation: the tool did not know our exam format, so I asked Morgan.',link_url:null,submitted_at:iso(-95)});
...
function fixtures(now) {
 ...
 s.courses.push({id:IDS.career,title:'Career Ready: Tell Your Story',description:'Turn a class project into a clear portfolio story: what you made, who it helped, and what you learned.',category:'Career',status:'published',image_url:null});
 s.modules.push(
  {id:'50000000-0000-4000-8000-000000000011',course_id:IDS.career,title:'Name what you made and who it helped',position:1,body:'Start with the work, not the job title. Say what you made, who it was for, and one result you can point to.\n\nPractice: write two sentences about a class project using that order.',resource_url:null,question:null,options:[],answer_index:null,assignment_prompt:null},
  {id:'50000000-0000-4000-8000-000000000012',course_id:IDS.career,title:'Turn it into a portfolio entry',position:2,body:'A strong entry has a title, a one-line summary, the skills you used, and a link or image. Keep it short enough to read in thirty seconds.',resource_url:null,question:null,options:[],answer_index:null,assignment_prompt:null});
 s.enrollments=[{course_id:IDS.course,user_id:IDS.student,enrolled_at:iso(-120),completed_at:null,credential_id:null},{course_id:IDS.career,user_id:IDS.student,enrolled_at:iso(-30240),completed_at:iso(-20160),credential_id:'c7000000-0000-4000-8000-000000000001'},{course_id:IDS.course,user_id:IDS.other,enrolled_at:iso(-2880),completed_at:null,credential_id:null}];
 s.progress=[...s.modules.slice(0,2).map(m=>({module_id:m.id,user_id:IDS.other,completed_at:iso(-100)})),...s.modules.filter(m=>m.course_id===IDS.career).map(m=>({module_id:m.id,user_id:IDS.student,completed_at:iso(-20160)}))];s._checks=s.progress.map(p=>({module_id:p.module_id,user_id:p.user_id}));
 ...
 s.cohort_members=[{cohort_id:IDS.cohort,user_id:IDS.student,active:true,joined_at:iso(-10080)},{cohort_id:IDS.cohort,user_id:IDS.classmate,active:true,joined_at:iso(-10080)},{cohort_id:IDS.otherCohort,user_id:IDS.other,active:true,joined_at:iso(-10080)}];
