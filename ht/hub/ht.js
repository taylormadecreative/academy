...
(function () {
  ...
  R.intro = function (b) {
    var art = '';
    if (b.video) art = h`<div class="art"><video muted loop playsinline autoplay preload="none" poster="${b.poster || ''}" aria-label="${b.imageAlt || ''}"><source src="${b.video}" type="video/mp4"></video></div>`;
  ...
  R.cards = function (b) {
    var inner = '<div class="cards">' + b.items.map(function (c) {
      ...
      return '<' + tag + ' class="card"' + (c.href ? h` href="${c.href}"` : '') + '>' + (c.img ? h`<div class="img"><img src="${c.img}" alt="${c.alt || ''}" loading="lazy" decoding="async"></div>` : '') +
  ...
  R.agenda = function (b) {
    ...
    var tools = '<div style="display:flex;gap:8px;flex-wrap:wrap;margin-top:16px">' + (b.ics !== false ? '<button class="pill" type="button" data-ics>' + ICONS.calendar.replace('viewBox', 'width="14" height="14" viewBox') + ' Add to calendar</button>' : '') + '<button class="pill ghost" type="button" onclick="window.print()">Print the agenda</button></div>';
  ...
  R.people = function (b) {
    var inner = b.items.map(function (p) {
      return '<div class="person">' + h`<span class="av${p.gold ? ' g' : ''}">${p.init || initials(p.name)}</span><div><b>${p.name}</b><span>${[p.role, p.org].filter(Boolean).join(' · ')}</span></div>` +
        '<div class="act">' + (p.tag ? chipHtml(p.tag, p.tagCls || 'soft') : '') + (b.dm ? '<button class="pill ghost" type="button" data-dm="' + esc(p.name) + '">Message</button>' : '') + '</div></div>';
    }).join('');
    return card(b, inner);
  };
  R.directory = function (b) {
    ...
    var inner = (b.search !== false ? '<div class="search"><input type="search" placeholder="Search by name, program, or skill" aria-label="Search the directory" data-search></div>' : '') + '<div data-list>' + rows + '</div><div class="empty" data-none hidden>No one matches that yet.</div>';
    return card(b, inner);
  };
  ...
  R.materials = function (b) {
    var inner = b.items.map(function (m) {
      return '<div class="mat">' + h`<span class="k">${m.kind || 'DOC'}</span><div><b>${m.title}</b>` + (m.sub ? h`<span>${m.sub}</span>` : '') + '</div>' + (m.href ? h`<a class="lnk" href="${m.href}">Open</a>` : (m.restricted ? chipHtml('Restricted', 'soft') : '')) + '</div>';
    }).join('');
    return card(b, inner);
  };
  R.tracks = function (b) {
    var inner = b.items.map(function (t) {
      ...
      return '<div class="track"><div class="th"><div>' + h`<h3>${t.title}</h3>` + (t.text ? h`<p>${t.text}</p>` : '') + '</div>' + (t.tag ? chipHtml(t.tag, t.tagCls || '') : '') + '</div>' +
        '<div class="prog"><i data-w="' + pct + '"></i></div>' + h`<div class="pm"><span>${done} of ${total} sessions</span><span>${pct}%</span></div>` +
  ...
  R.feed = function (b) {
    var chans = (b.channels || []).map(function (c, i) { return '<button class="pill' + (i === 0 ? ' on' : ' ghost') + '" type="button" data-chan="' + esc(c) + '">' + esc(c) + '</button>'; }).join('');
    var posts = b.posts.map(function (p, i) { return postHtml(p, i); }).join('');
    var inner = '<div class="chans" data-chans>' + chans + '</div><form class="composer" data-composer><input placeholder="Post to the channel" aria-label="Write a post" maxlength="280"><button class="btn ht sm" type="submit">Post</button></form><div data-posts>' + posts + '</div>';
    return card(b, inner);
  };
  function postHtml(p, i) {
    return '<div class="post" data-chan="' + esc(p.chan || '') + '"><div class="ph">' + h`<span class="av">${p.init || initials(p.who)}</span><div><b>${p.who}</b><br><span>${[p.chan, p.when].filter(Boolean).join(' · ')}</span></div>` + '</div>' + h`<p>${p.text}</p>` + '<div class="acts"><button type="button" data-like="' + i + '">♥ <span>' + (p.likes || 0) + '</span></button><span>Reply</span></div></div>';
  }
  R.chat = function (b) {
    var msgs = (b.seed || []).concat(get('chat:' + b.room, []));
    var inner = '<div class="chat"><div class="scroll" data-scroll>' + msgs.map(bub).join('') + '</div><form data-chat="' + esc(b.room) + '"><input placeholder="Say something to the room" aria-label="Message" maxlength="400"><button type="submit">Send</button></form></div>';
  ...
  R.checkin = function (b) {
    ...
    var inner = h`<p style="margin-bottom:12px"><b>${b.session}</b>` + (b.sub ? h` · ${b.sub}` : '') + '</p><div class="ci" data-checkin="' + esc(b.session) + '" data-code="' + esc(b.code) + '">' +
      (done ? '<div class="ok"><i>' + ICONS.check + '</i>You are checked in.</div>' : '<input placeholder="CODE" maxlength="12" aria-label="Check-in code"><button class="btn ht sm" type="button">Check in</button><span class="msg" style="font-size:13px;font-weight:600"></span>') +
      (b.hint ? h`<div class="hint">${b.hint}</div>` : '') + '</div>';
    return card(b, inner);
  };
  R.replays = function (b) {
    var inner = '<div class="reps">' + b.items.map(function (r) { return '<div class="rep">' + h`<img src="${r.poster}" alt="" loading="lazy" decoding="async">` + (r.len ? h`<span class="len">${r.len}</span>` : '') + '<div class="cap">' + h`<b>${r.title}</b><span>${[r.date, r.tag].filter(Boolean).join(' · ')}</span>` + '</div></div>'; }).join('') + '</div>';
    return card(b, inner);
  };
  R.player = function (b) {
    var now = b.now ? '<div class="nowbar">' + (b.live ? '<span class="chip live"><i></i>Live</span>' : chipHtml('Rehearsal loop', 'soft')) + h`<b>${b.now.title}</b>` + (b.now.who ? h`<span>${b.now.who}</span>` : '') + (b.now.when ? h`<span>${b.now.when}</span>` : '') + '</div>' : '';
    var inner = '<div class="player" data-player data-stream="' + esc(b.stream || '/assets/live-demo/stream.m3u8') + '">' +
      ...
      '<div class="poster" style="background-image:url(' + esc(b.poster || '/assets/live-demo/poster.jpg') + ')"><button type="button">▶&nbsp; Watch</button></div>' +
  ...
  R.timeline = function (b) {
    var inner = '<div class="tl">' + b.items.map(function (t) { return '<div class="tli' + (t.done ? ' done' : '') + '">' + h`<em>${t.when}</em><b>${t.title}</b>` + (t.text ? h`<p>${t.text}</p>` : '') + '</div>'; }).join('') + '</div>';
    return card(b, inner);
  };
  ...
  R.split = function (b) {
    var inner = '<div class="split' + (b.side === 'left' ? ' r' : '') + '"><div>' + (b.kicker ? h`<div class="k" style="font-size:12px;font-weight:700;letter-spacing:.14em;text-transform:uppercase;color:var(--ht-maroon);margin-bottom:8px">${b.kicker}</div>` : '') + h`<h3>${b.title}</h3>` + (b.text ? h`<p>${b.text}</p>` : '') +
  ...
  R.cta = function (b) { return '<section class="cta rv"' + (b.id ? h` id="${b.id}"` : '') + '><div>' + h`<h3>${b.title}</h3>` + (b.text ? h`<p>${b.text}</p>` : '') + '</div><div class="ctas">' + btn(b.primary) + (b.secondary ? btn(Object.assign({ style: 'ht-gold' }, b.secondary)) : '') + '</div></section>'; };
  R.table = function (b) { return card(b, '<div style="overflow-x:auto"><table class="tbl"><thead><tr>' + b.cols.map(function (c) { return h`<th>${c}</th>`; }).join('') + '</tr></thead><tbody>' + b.rows.map(function (r) { return '<tr>' + r.map(function (c, i) { return i === 0 ? h`<td><b>${c}</b></td>` : h`<td>${c}</td>`; }).join('') + '</tr>'; }).join('') + '</tbody></table></div>'); };
  R.install = function (b) {
    var inner = '<div class="install"><img src="/ht/img/icon-192.png" alt="HT Hub app icon"><div><b>Put the HT Hub on your phone</b><span>Open this page in Safari, tap Share, then <b>Add to Home Screen</b>. It opens like an app, with its own icon.</span></div></div>';
  ...
  function tabsHtml(active) {
    var t = '<a class="ht-tab home' + (active === 'home' ? ' on' : '') + '" href="' + HT.site.hub + '">' + ICONS.home + ' Home</a>';
    HT.order.forEach(function (k) { var s = HT.spaces[k]; if (s) t += h`<a class="ht-tab${k === active ? ' on' : ''}" href="${HT.site.hub + k + '/'}">${s.title}</a>`; });
    return t;
  }
  function render(key) {
    ...
    var head = '<div class="ht-head"><div class="hub-wrap"><div>' +
      '<div class="ht-lockup"><img src="/ht/img/ht-wordmark-maroon.png" alt="Huston-Tillotson University"><span class="x">×</span><span class="tma"><img src="/assets/logo-nav.webp" alt="">Taylormade Academy</span></div>' +
    ...
    var body = '<main class="ht-main"><div class="hub-wrap"><div class="ht-grid' + (side.length ? '' : ' one') + '">' + '<div class="ht-col">' + main.join('') + '</div>' + (side.length ? '<div class="ht-col">' + side.join('') + '</div>' : '') + '</div></div></main>';
  ...
  function wire(root, space) {
    ...
    root.querySelectorAll('[data-checkin]').forEach(function (box) {
      ...
      function go() { var v = (inp.value || '').trim().toUpperCase(); if (!v) { msg.textContent = 'Type the code from the screen.'; msg.style.color = '#8f0000'; return; }
        if (v === String(box.getAttribute('data-code')).toUpperCase()) { set('checkin:' + box.getAttribute('data-checkin'), true); box.innerHTML = '<div class="ok"><i>' + ICONS.check + '</i>You are checked in.</div>' + (box.querySelector('.hint') ? box.querySelector('.hint').outerHTML : ''); }
        else { msg.textContent = 'That code did not match. Try again.'; msg.style.color = '#8f0000'; } }
    ...
    root.querySelectorAll('form[data-chat]').forEach(function (f) {
      ...
      f.addEventListener('submit', function (e) { e.preventDefault(); var inp = f.querySelector('input'), t = inp.value.trim(); if (!t) return;
        ...
        scroll.insertAdjacentHTML('beforeend', bub(m)); inp.value = ''; scroll.scrollTop = scroll.scrollHeight; });
    });
    /* DM buttons (local) */
    root.querySelectorAll('[data-dm]').forEach(function (b) { b.addEventListener('click', function () { var n = b.getAttribute('data-dm'); b.textContent = 'Message sent'; b.disabled = true; b.classList.add('on'); set('dm:' + n, true); }); });
    /* feed */
    root.querySelectorAll('[data-composer]').forEach(function (f) {
      ...
      if (chans) chans.querySelectorAll('[data-chan]').forEach(function (c) { c.addEventListener('click', function () { chans.querySelectorAll('[data-chan]').forEach(function (x) { x.classList.remove('on'); x.classList.add('ghost'); }); c.classList.add('on'); c.classList.remove('ghost'); cur = c.getAttribute('data-chan'); filter(); }); });
      f.addEventListener('submit', function (e) { e.preventDefault(); var inp = f.querySelector('input'), t = inp.value.trim(); if (!t) return; list.insertAdjacentHTML('afterbegin', postHtml({ who: HT.me || 'You', chan: cur, when: 'Just now', text: t, likes: 0 }, 'x')); inp.value = ''; wireLikes(list); });
      wireLikes(list); filter();
    });
    function wireLikes(list) { list.querySelectorAll('[data-like]').forEach(function (b) { if (b._w) return; b._w = 1; b.addEventListener('click', function () { var s = b.querySelector('span'); var on = b.classList.toggle('on'); s.textContent = (+s.textContent) + (on ? 1 : -1); b.style.color = on ? '#8f0000' : ''; }); }); }
    /* directory search */
    root.querySelectorAll('[data-search]').forEach(function (inp) {
      ...
      inp.addEventListener('input', function () { var q = inp.value.trim().toLowerCase(), n = 0; rows.forEach(function (r) { var ok = !q || r.getAttribute('data-q').indexOf(q) > -1; r.hidden = !ok; if (ok) n++; }); none.hidden = n > 0; });
    });
    /* ics */
    root.querySelectorAll('[data-ics]').forEach(function (b) {
      b.addEventListener('click', function () { var blk = (space.blocks || []).filter(function (x) { return x.type === 'agenda'; })[0]; if (!blk) return; downloadICS(blk); });
    });
    /* player */
  ...
  function downloadICS(blk) {
    var lines = ['BEGIN:VCALENDAR', 'VERSION:2.0', 'PRODID:-//Huston-Tillotson x Taylormade Academy//HT Hub//EN', 'CALSCALE:GREGORIAN'];
    (blk.days || []).forEach(function (d) { if (!d.date) return; d.items.forEach(function (it, i) {
      var s = parseTime(it.time), e = it.end ? parseTime(it.end) : [s[0] + 1, s[1]]; var ds = d.date.replace(/-/g, '');
      lines.push('BEGIN:VEVENT', 'UID:' + ds + '-' + i + '@ht.taylormadeacademy.com', 'DTSTAMP:' + ds + 'T' + pad(s[0]) + pad(s[1]) + '00', 'DTSTART:' + ds + 'T' + pad(s[0]) + pad(s[1]) + '00', 'DTEND:' + ds + 'T' + pad(Math.min(e[0], 23)) + pad(e[1]) + '00',
        'SUMMARY:' + String((blk.event && blk.event.name ? blk.event.name + ' · ' : '') + it.title).replace(/,/g, '\\,'), 'LOCATION:' + String(it.where || (blk.event && blk.event.place) || '').replace(/,/g, '\\,'), 'END:VEVENT'); }); });
  ...
  function startPlayer(p) {
    var src = p.getAttribute('data-stream'), poster = p.querySelector('.poster'); poster.querySelector('button').textContent = 'Loading…';
    var v = document.createElement('video'); v.playsInline = true; v.controls = true; v.loop = true; v.setAttribute('aria-label', 'Live room video');
    function fail(msg) { poster.remove(); p.insertAdjacentHTML('beforeend', '<div class="fail">' + esc(msg) + '</div>'); }
    function go() { p.insertBefore(v, p.querySelector('.wm')); poster.remove(); p.classList.add('playing'); v.play().catch(function () { v.muted = true; v.play().catch(function () {}); }); }
    if (v.canPlayType('application/vnd.apple.mpegurl')) { v.src = src; go(); return; }
    import('https://cdn.jsdelivr.net/npm/hls.js@1.5.13/+esm').then(function (m) { var Hls = m.default; if (!Hls.isSupported()) { fail('This browser cannot play live video. Try Chrome, Safari, or Edge.'); return; }
      var hl = new Hls({ liveSyncDurationCount: 3 }); hl.on(Hls.Events.ERROR, function (_, d) { if (d.fatal) { if (d.type === 'mediaError') hl.recoverMediaError(); else fail('The stream could not load.'); } }); hl.loadSource(src); hl.attachMedia(v); go(); })
      .catch(function () { fail('The video player could not load. Check your connection or any blocker.'); });
  }
