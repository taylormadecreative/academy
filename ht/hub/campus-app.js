...
export async function mountCampus(view='home') {
 ...
 function render() {
  ...
  const contextualDesc=view==='courses'&&route?.cohort?(route.params.has('assignment')?(isStaff?'Review submissions, grade with the rubric, and publish feedback.':'Read the assignment instructions, due dates, submission history, and instructor feedback.'):({overview:'Course details and the next task for this section.',modules:'Open course learning materials and follow modules in order.',assignments:'Read assignment instructions, submit work, and follow published feedback.',grades:'Review published grades and feedback, or manage the section gradebook.',people:'Find your instructor and class roster.'}[route.params.get('tab')||'overview']||desc))
