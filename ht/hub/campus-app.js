...
const [{createCampusStore},{renderStudent,bindStudent},{renderStaff,bindStaff},{renderClassrooms,bindClassrooms},{renderAcademics,bindAcademics},{renderOnboarding,renderOnboardingPrompt,renderPageOrientation,bindOnboarding},{createOnboardingProgress,onboardingSteps},{renderLeadership,bindLeadership,leadershipViews},{renderSuccess,bindSuccess},{renderTrust,bindTrust}] = await Promise.all([
 import('./campus-store.js'+assetStamp),import('./campus-student.js'+assetStamp),import('./campus-staff.js'+assetStamp),import('./campus-classrooms.js'+assetStamp),import('./campus-academics.js'+assetStamp),import('./campus-onboarding.js'+assetStamp),import('./campus-onboarding-progress.js'+assetStamp),import('./campus-leadership.js'+assetStamp),import('./campus-success.js'+assetStamp),import('./campus-trust.js'+assetStamp)
]);
...
export async function mountCampus(view='home') {
 ...
 function navigation() {
  ...
  const more=`<details class="campus-nav-more"><summary${moreCurrent?' aria-current="page"':''}>${icon('grid')}<span>More</span><span class="campus-nav-more-chevron" aria-hidden="true">⌄</span></summary><div class="campus-nav-more-panel" aria-label="More campus destinations">${moreLinks.map(args=>navLink(...args)).join('')}<a href="${href('/ht/hub/calendar/')}">${icon('calendar')}<span>Academic calendar</span></a></div></details>`;
  return `<nav class="campus-nav campus-nav-communication" aria-label="Main navigation"><div class="campus-nav-inner">${links.map(args=>navLink(...args)).join('')}<div class="campus-nav-end">${more}</div></div></nav>`;
 }
 ...
 function render() {
  ...
  const title=view==='home'?(state.member?`Good ${new Date().getHours()<12?'morning':new Date().getHours()<17?'afternoon':'evening'}, ${state.member.display_name.split(' ')[0]}.`:'Your day on the Hill.'):currentPageName();
  const route=courseRoute();
  ...
  else if(state.mode!=='unavailable'&&view!=='welcome')cleanup=(view==='success'?bindSuccess:view==='trust'?bindTrust:leadershipViews(view,ctx())?bindLeadership:view==='courses'?bindAcademics:view==='live'?bindClassrooms:['staff','support','insights'].includes(view)?bindStaff:bindStudent)(view,root,ctx());
