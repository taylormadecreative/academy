...
export function academicGradeSummary(state, cohortId, studentId) {
  ...
  const items = assignments.map(assignment => {
    ...
    if (currentGrade?.disposition === 'excused') { excused++; stateLabel = 'Excused'; status = 'excused'; }
    else if (currentGrade) { earned += number(currentGrade.score); possible += number(assignment.points_possible); graded++; stateLabel = `${number(currentGrade.score)} / ${number(assignment.points_possible)}`; status = 'graded'; }
    else { pending++; stateLabel = latestAttempt ? publishedGrade ? 'Resubmitted · awaiting grade' : 'Submitted · awaiting grade' : 'Not submitted'; status = latestAttempt ? 'waiting' : 'missing'; }
    return { assignment, latestAttempt, publishedGrade, currentGrade, stateLabel, status, ...extensionFor(state, assignment, studentId) };
  });
...
/** "Fri, Sep 25 at 11:59 PM" (year only when it is not this year). */
function when(ctx, value) {
  if (!Number.isFinite(stamp(value))) return '';
  const d = new Date(value), day = d.toLocaleDateString('en-US', { weekday: 'short', month: 'short', day: 'numeric', ...(d.getFullYear() !== new Date().getFullYear() ? { year: 'numeric' } : {}) });
  return `${day} at ${ctx.formatTime(value)}`;
}
/** "Sep 23 at 8:59 AM" */
function shortWhen(ctx, value) { return Number.isFinite(stamp(value)) ? `${ctx.formatDate(value)} at ${ctx.formatTime(value)}` : ''; }
/**
 * What a student sees for one assignment: not turned in yet, waiting for a grade, or graded.
 * Returns plain text; callers escape it.
 */
function studentStatus(ctx, item, cohort) {
  if (!item) return { label: 'Not turned in yet', note: '' };
  if (item.status === 'graded') return { label: item.stateLabel, note: '' };
  if (item.status === 'excused') return { label: 'Excused', note: 'Excused work isn’t counted in your grade.' };
  if (item.status === 'waiting') return { label: 'Waiting for a grade', note: 'Waiting for a grade. It doesn’t count against you.' };
  const now = Date.now(), due = when(ctx, item.due_at), closes = when(ctx, item.closes_at), teacher = firstName(name(ctx, cohort?.instructor_id));
  if (Number.isFinite(stamp(item.closes_at)) && now > stamp(item.closes_at)) return { label: 'Not turned in', note: `Not turned in · closed ${closes}. Ask ${teacher} about more time.` };
  if (Number.isFinite(stamp(item.due_at)) && now > stamp(item.due_at)) return { label: 'Not turned in yet', note: `Not turned in yet · was due ${due}${closes ? `. You can still turn it in until ${closes}` : ''}.` };
  return { label: 'Not turned in yet', note: due ? `Not turned in yet · due ${due}` : 'Not turned in yet · no due date' };
}
...
const firstName = value => String(value || '').trim().split(/\s+/)[0] || 'your instructor';
function summaryCard(ctx, summary, cohort) {
  const label = summary.final ? 'Final grade' : 'Current grade', teacher = firstName(name(ctx, cohort?.instructor_id));
  const count = summary.items.filter(item => item.latestAttempt && !item.currentGrade).length;
  const missing = summary.items.filter(item => item.status === 'missing').length;
  const waiting = (count ? `<p class="campus-muted">${count === 1 ? '1 assignment is' : `${count} assignments are`} waiting for a grade. Work waiting for a grade doesn’t count against you.</p>` : '') + (missing ? `<p class="campus-muted">${missing === 1 ? '1 assignment isn’t' : `${missing} assignments aren’t`} turned in yet.</p>` : '');
  if (summary.currentPercent === null) return `<section class="campus-panel campus-academic-grade-summary"><p class="campus-eyebrow">${label}</p><h3 class="campus-academic-grade-empty">No grade yet</h3><p>You’ll see it here once ${esc(ctx, teacher)} grades your work.</p>${waiting}</section>`;
  return `<section class="campus-panel campus-academic-grade-summary"><p class="campus-eyebrow">${label}</p><h3 class="campus-academic-grade-number">${summary.currentPercent.toFixed(1)}%</h3><p><strong>${points(summary.earned)} / ${points(summary.possible)} points</strong> from ${summary.graded} graded ${summary.graded === 1 ? 'assignment' : 'assignments'}.</p>${waiting}${summary.excused ? `<p class="campus-muted">Excused work isn’t counted.</p>` : ''}</section>`;
}
/** Students whose latest attempt has no published grade yet, oldest submission first. */
function waitingForGrade(ctx, cohort, assignment) {
  return reviewStudents(ctx, cohort, assignment).filter(id => activeLearner(ctx, cohort, id)).map(id => ({ id, item: academicGradeSummary(ctx.state, cohort.id, id).items.find(entry => entry.assignment.id === assignment.id) })).filter(entry => entry.item?.latestAttempt && !entry.item.currentGrade).sort((a, b) => (stamp(a.item.latestAttempt.submitted_at) || 0) - (stamp(b.item.latestAttempt.submitted_at) || 0));
}
const waitingLabel = count => `${count} ${count === 1 ? 'submission' : 'submissions'} waiting for a grade`;
...
function overview(ctx, cohort, manage) {
  ...
  const nextStatus = next && !manage ? studentStatus(ctx, academicGradeSummary(ctx.state, cohort.id, uid(ctx)).items.find(item => item.assignment.id === next.id), cohort) : null;
  const about = `<section class="campus-panel"><h3>About this course</h3>${paragraphs(ctx, cohort.description || 'Your lessons, assignments, and feedback all live here.')}<div class="campus-card-actions">${link(ctx, href(ctx, cohort, 'assignments'), 'Open assignments', 'campus-button')}<a class="campus-button campus-button-secondary" href="${esc(ctx, ctx.href('live', { cohort: cohort.id }))}">Open classroom</a></div></section>`;
...
function modules(ctx, cohort) {
  ...
  return `${ada ? '<div class="campus-academic-layout">' : ''}<section class="campus-panel"><div class="campus-section-head"><div><p class="campus-eyebrow">Connected learning pathway</p><h3>${esc(ctx, course.title)}</h3></div><a class="campus-button campus-button-secondary" href="${esc(ctx, ctx.href('learn'))}#course-${esc(ctx, course.id)}">Open pathway activities</a></div><p>${esc(ctx, course.description)}</p><p class="campus-muted">Finishing these lessons doesn’t change your course grade. Your grade comes from assignments.</p>${modules.length ? `<div class="campus-list">${modules.map((item, index) => `<details class="campus-academic-module" id="module-${index + 1}"><summary><span class="campus-eyebrow">Module ${index + 1}</span> <strong>${esc(ctx, item.title)}</strong></summary>${paragraphs(ctx, item.body)}<a href="${esc(ctx, ctx.href('learn'))}#module-${esc(ctx, item.id)}">Open this learning activity</a></details>`).join('')}</div>` : empty(ctx, 'No modules published yet', 'Your instructor is preparing this pathway.')}</section>${ada ? `<aside class="campus-stack">${ada}</aside></div>` : ''}`;
}
...
/** "brief", "essay", … from the assignment title; "work" when the title names none. */
function workNoun(assignment) {
  const match = String(assignment?.title || '').toLowerCase().match(/\b(brief|essay|reflection|paper|report|presentation|proposal|plan|project)\b(?!.*\b(brief|essay|reflection|paper|report|presentation|proposal|plan|project)\b)/);
  return match ? match[1] : 'work';
}
/** The grade button says what it will do: publish, update what the student already sees, or keep a draft. */
export function gradeSubmitLabel(first, visibility, wasPublished) {
  if (visibility !== 'published') return 'Save draft';
  return wasPublished ? `Update ${possessive(first)} grade` : `Publish to ${first}`;
}
function gradingForm(ctx, cohort, assignment, studentId) {
  ...
  // The control starts on the current revision's visibility, so editing a published grade keeps it published.
  const visibility = applicable?.status === 'published' ? 'published' : 'draft', shown = visibility === 'published', first = firstName(name(ctx, studentId));
  ...
  return `<form data-academic-form="grade" data-cohort-id="${esc(ctx, cohort.id)}" data-assignment-id="${esc(ctx, assignment.id)}" data-student-id="${esc(ctx, studentId)}"${rubric ? ` data-student-name="${esc(ctx, name(ctx, studentId))}"` : ''}><input type="text" name="expected_revision" value="${number(latestGrade?.revision)}" hidden readonly><input type="text" name="attempt_id" value="${esc(ctx, latest?.id || '')}" hidden readonly>${applicable ? `<p class="campus-academic-published"><strong>${applicable.status === 'published' ? `Published ${esc(ctx, ctx.formatDate(applicable.published_at || applicable.created_at))}` : `Draft saved ${esc(ctx, ctx.formatDate(applicable.created_at || applicable.published_at))} · only instructors see it`}</strong><span>${applicable.disposition === 'excused' ? 'Excused' : `${points(applicable.score)} / ${points(assignment.points_possible)} points`}. ${applicable.status === 'published' ? `${esc(ctx, firstName(name(ctx, studentId)))} can see this grade. Edit below to change it.` : 'Edit below, then publish when ready.'}</span></p>` : ''}${select(ctx, 'Result', 'disposition', [['graded', 'Score this assignment'], ['excused', 'Excused (does not count)']], applicable?.disposition || 'graded')}${rubricHtml}${field(ctx, `Score (out of ${number(assignment.points_possible)})`, 'score', applicable?.disposition === 'graded' ? applicable.score : '', 'number', `min="0" max="${number(assignment.points_possible)}" step="0.01" required`)}${!latest ? '<p class="campus-muted">No attempt has been submitted. You may excuse this assignment or publish an explicit zero with explanatory feedback.</p>' : `<p class="campus-muted">Grading ${esc(ctx, possessive(firstName(name(ctx, studentId))))} ${esc(ctx, workNoun(assignment))}${number(latest.attempt_no) > 1 ? ` (attempt ${number(latest.attempt_no)})` : ''}, turned in ${esc(ctx, shortWhen(ctx, latest.submitted_at))}.</p>`}${textarea(ctx, 'Instructor feedback', 'feedback', applicable?.feedback || '', 'maxlength="12000"')}${rubric ? tutor.renderDraftButton() : ''}${select(ctx, 'Grade visibility', 'status', [['draft', 'Draft (only instructors see it)'], ['published', 'Publish to student']], visibility)}<p class="campus-member-notice campus-academic-unpublish" data-grade-unpublish-warning${shown && visibility === 'draft' ? '' : ' hidden'}>${esc(ctx, first)} will stop seeing this grade until you publish again.</p><p class="campus-muted">Draft feedback stays private. Publishing a grade notifies the student. A newer submission requires a new review.</p><div class="campus-card-actions"><button class="campus-button" type="submit" data-grade-submit data-label-draft="Save draft" data-label-publish="${esc(ctx, `Publish to ${first}`)}" data-label-update="${esc(ctx, `Update ${possessive(first)} grade`)}"${shown ? ' data-was-published' : ''}>${esc(ctx, gradeSubmitLabel(first, visibility, shown))}</button><button class="campus-button campus-button-secondary" type="button" data-academic-discard-review>Start over</button></div><p class="campus-muted">Start over clears your unsaved score and feedback, then loads the newest work and grade.</p>${formStatus()}</form>`;
}
...
function teacherAssignment(ctx, cohort, assignment, route) {
  const waiting = assignment.status === 'published' ? waitingForGrade(ctx, cohort, assignment) : [], waitingIds = new Set(waiting.map(entry => entry.id));
  // Work waiting for a grade comes first, and opens first.
  const students = [...waiting.map(entry => entry.id), ...reviewStudents(ctx, cohort, assignment).filter(id => !waitingIds.has(id))];
  const selected = route.student ? students.find(id => id === route.student) : students[0];
  const callout = waiting.length ? `<div class="campus-academic-waiting"><p><strong>${waitingLabel(waiting.length)}</strong></p><ul>${waiting.map(({ id, item }) => `<li><a href="${esc(ctx, href(ctx, cohort, 'assignments', { assignment: assignment.id, student: id }))}" data-academic-nav${id === selected ? ' aria-current="true"' : ''}><strong>${esc(ctx, name(ctx, id))}</strong><span>Attempt ${number(item.latestAttempt.attempt_no)} · turned in ${date(ctx, item.latestAttempt.submitted_at)}</span></a></li>`).join('')}</ul></div>` : '';
  return `<div class="campus-stack"><div class="campus-academic-assignment-meta"><p class="campus-eyebrow">${assignment.status === 'draft' ? 'Draft · only instructors can see it' : 'Published'} · ${points(assignment.points_possible)} points</p><p class="campus-muted">Due ${date(ctx, assignment.due_at, 'No due date')}</p>${assignmentEditor(ctx, cohort, assignment)}</div><section class="campus-panel campus-academic-review"><div class="campus-section-head"><div><p class="campus-eyebrow">Instructor review</p><h3>Submissions & grading</h3></div>${link(ctx, href(ctx, cohort, 'grades'), 'Open gradebook')}</div>${callout}${students.length ? `<nav class="campus-academic-review-list" aria-label="Choose a student to review">${students.map(id => `<a href="${esc(ctx, href(ctx, cohort, 'assignments', { assignment: assignment.id, student: id }))}" data-academic-nav${id === selected ? ' aria-current="true"' : ''}>${esc(ctx, name(ctx, id))}${waitingIds.has(id) ? '<small>· needs a grade</small>' : ''}${activeLearner(ctx, cohort, id) ? '' : ' · inactive enrollment'}</a>`).join('')}</nav>` : '<p class="campus-muted">Add students through Manage sections & enrollment to start reviewing coursework.</p>'}${route.student && !selected ? '<p class="campus-member-notice">That student’s record is not available in this section.</p>' : ''}</section>${selected ? `<div class="campus-stack campus-academic-grading">${latestSubmission(ctx, assignment, selected)}<section class="campus-panel"><h3>Review ${esc(ctx, name(ctx, selected))}</h3>${gradingForm(ctx, cohort, assignment, selected)}${extensionForm(ctx, cohort, assignment, selected)}</section>${attemptHistory(ctx, attemptsFor(ctx.state, assignment.id, selected), gradesFor(ctx.state, assignment.id, selected), attemptsFor(ctx.state, assignment.id, selected)[0]?.id)}</div>` : ''}</div>`;
}
...
function assignments(ctx, cohort, manage, route) {
  ...
  if (route.assignment) {
    ...
    return manage ? teacherAssignment(ctx, cohort, assignment, route) : studentAssignment(ctx, cohort, assignment);
  }
...
function studentGrades(ctx, cohort) {
  ...
  return `<div class="campus-academic-layout"><div class="campus-stack"><section class="campus-panel"><h3>Your assignments and grades</h3>${summary.items.length ? `<div class="campus-list">${summary.items.map(item => `<article class="campus-academic-grade-row"><div class="campus-section-head"><h4>${link(ctx, href(ctx, cohort, 'assignments', { assignment: item.assignment.id }), item.assignment.title)}</h4><strong>${esc(ctx, item.status === 'missing' ? 'To do' : studentStatus(ctx, item, cohort).label)}</strong></div>${item.currentGrade ? paragraphs(ctx, item.currentGrade.feedback || 'No written feedback.') + rubricResult(ctx, item.assignment, item.currentGrade) : `<p class="campus-muted">${esc(ctx, studentStatus(ctx, item, cohort).note)}</p>`}</article>`).join('')}</div>` : empty(ctx, 'No grades yet', 'Your published assignments and instructor feedback will appear here.')}</section></div><aside>${summaryCard(ctx, summary, cohort)}</aside></div>`;
}
...
export function renderAcademics(view, ctx) {
  ...
  // The page h1 already names the section (or assignment), so this is context, not a second title.
  const course = find(ctx.state, 'courses', cohort.course_id), section = String(cohort.title || '').split(' · ').slice(1).join(' · ');
  const facts = [['Instructor', cohort.instructor_id === uid(ctx) ? `${name(ctx, cohort.instructor_id)} (you)` : name(ctx, cohort.instructor_id)], ['Section', section || cohort.title], ...(course?.status === 'published' ? [['Course', course.title]] : [])];
  return `<div data-campus-academics><section class="campus-academic-context" aria-label="About this section"><dl>${facts.map(([term, value]) => `<div><dt>${esc(ctx, term)}</dt><dd>${esc(ctx, value)}</dd></div>`).join('')}</dl><a class="campus-button campus-button-secondary campus-button-small" href="${esc(ctx, ctx.href('live', { cohort: cohort.id }))}">Classroom & recordings</a></section>${cohort.status !== 'active' ? '<p class="campus-member-notice">This section is archived. Coursework is available as history; editing and submissions are closed.</p>' : ''}<nav class="campus-academic-tabs" aria-label="Course navigation">${tabs.map(tab => `<a href="${esc(ctx, href(ctx, cohort, tab))}" data-academic-nav${route.tab === tab ? ' aria-current="page"' : ''}>${tab === 'grades' && manage ? 'Gradebook' : tab[0].toUpperCase() + tab.slice(1)}</a>`).join('')}</nav>${content}</div>`;
}
...
export function bindAcademics(view, root, ctx) {
  ...
  const syncGrade = form => { if (form?.dataset.academicForm !== 'grade') return; const excused = form.elements.disposition.value === 'excused'; form.elements.score.disabled = excused; form.elements.score.required = !excused;
    const button = form.querySelector('[data-grade-submit]'), visibility = form.elements.status?.value, warning = form.querySelector('[data-grade-unpublish-warning]');
    if (button) button.textContent = visibility === 'published' ? button.hasAttribute('data-was-published') ? button.dataset.labelUpdate : button.dataset.labelPublish : button.dataset.labelDraft;
    if (warning) warning.hidden = !(button?.hasAttribute('data-was-published') && visibility === 'draft');
  };
  const click = async event => {
    ...
    await ctx.refresh();
    if (openHashTarget()) return;
    const heading = root.querySelector('[data-academic-focus]') || globalThis.document?.querySelector('.campus-page-head h1');
  ...
  // Kept off form.dataset: the app keys saved drafts by dataset, so a dataset change would orphan
  // an older draft that then comes back after a publish.
  const adaDrafts = new WeakMap();
  const draftWithAda = form => {
    ...
    const current = feedback.value.trim(), previous = adaDrafts.get(form) || '';
    feedback.value = !current || current === previous ? text : `${feedback.value.trimEnd()}\n\n${text}`;
    adaDrafts.set(form, text);
    touch(feedback);
