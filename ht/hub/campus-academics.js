...
function summaryCard(ctx, summary, cohort) {
  ...
  const waiting = count ? `<p class="campus-muted">${count === 1 ? '1 assignment is' : `${count} assignments are`} waiting for a grade. Work waiting for a grade doesn’t count against you.</p>` : '';
...
function studentGrades(ctx, cohort) {
  ...
  return `<div class="campus-academic-layout"><div class="campus-stack"><section class="campus-panel"><h3>Your published grades</h3>${summary.items.length ? `<div class="campus-list">${summary.items.map(item => `<article class="campus-academic-grade-row"><div class="campus-section-head"><h4>${link(ctx, href(ctx, cohort, 'assignments', { assignment: item.assignment.id }), item.assignment.title)}</h4><strong>${esc(ctx, item.stateLabel)}</strong></div>${item.currentGrade ? paragraphs(ctx, item.currentGrade.feedback || 'No written feedback.') + rubricResult(ctx, item.assignment, item.currentGrade) : '<p class="campus-muted">Waiting for a grade. It doesn’t count against you.</p>'}</article>`).join('')}</div>` : empty(ctx, 'No grades yet', 'Your published assignments and instructor feedback will appear here.')}</section></div><aside>${summaryCard(ctx, summary, cohort)}</aside></div>`;
}
