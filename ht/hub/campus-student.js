...
function homeView(ctx) {
  ...
  return `${guestNotice(state)}
    <section class="campus-hero campus-today-hero"><div class="campus-hero-copy"><p class="campus-eyebrow">${current ? 'Your learning, ready when you are' : 'Your day on the Hill'}</p><h2>${esc(current?.title || 'Find your next opportunity.')}</h2><p>${current ? 'Continue your course, check feedback, or see what is happening across campus.' : 'Learning, campus life, and the people who can help, together in one place.'}</p>${current ? courseProgress(current, ctx, true) : ''}<div class="campus-card-actions"><a class="campus-button" href="${esc(courseLink)}">${current ? 'Open your course' : 'Explore learning'}</a><a class="campus-button campus-button-secondary" href="${esc(href('messages'))}">Open messages</a></div></div><div class="campus-hero-aside"><p class="campus-eyebrow">${nextLabel}</p>${nextTitle ? `<h3>${esc(nextTitle)}</h3><p>${esc(dateText(nextTime, ctx, true))}</p><p>${esc(nextSession ? (nextSession.status === 'live' ? 'Your cohort classroom' : 'Cohort classroom · join when it begins') : nextEvent.location || nextEvent.office || 'Campus event')}</p><a href="${esc(nextLink)}">${nextSession ? 'Open classroom' : 'View event'} <span aria-hidden="true">→</span></a>` : `<h3>Good things happen together.</h3><p>Find your people, ask a question, or join a campus conversation.</p><a href="${esc(href('community'))}">Meet the community <span aria-hidden="true">→</span></a>`}</div></section>
