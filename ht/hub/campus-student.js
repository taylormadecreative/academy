...
function coCurricularRecord(ctx) {
  ...
  return `<section class="campus-panel badge-record" aria-labelledby="badge-record-title"><img class="badge-record-wordmark" src="/ht/img/ht-wordmark-maroon-900.png" alt="Huston-Tillotson University" width="220" height="33" decoding="async"><div class="campus-section-head"><div><p class="campus-eyebrow">Co-curricular record</p><h2 id="badge-record-title">${esc(name)}’s learning record</h2></div></div><p class="badge-record-issuer">Issued by ${esc(BADGE_ISSUER)}</p>${demo}${list}<div class="campus-card-actions badge-record-actions"><a class="campus-button campus-button-secondary campus-button-small" href="${esc(href('/ht/hub/career/'))}">See career portfolio</a><button type="button" class="campus-button campus-button-secondary campus-button-small" data-campus-action="printRecord"${earned.length ? '' : ' disabled'}>Print record</button></div></section>`;
}
