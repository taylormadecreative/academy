...
export async function mountRoomV2(o) {
  window.__mount = { mode: o.mode, target: o.target, facilitator: o.facilitator, meetingId: 'm-new' };
  ...
  const inClass = () => {
    const w = o.target.words, mark = o.target.mark || o.target.logo;
  ...
  const tell = (st, reason) => { if (gone) return; if (st === 'left' || st === 'ended') gone = true; o.onState(st, null, reason); };
