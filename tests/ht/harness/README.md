...
strip shows and the harness answers with `rejoinOk()` / `rejoinFail()`. Scenarios 5–5d cover the host's
...
polls and offers Rejoin once the room is live AGAIN). `window.__htRoomPollMs` shortens the page's poll
for that last one; only the harness sets it.
