...
(function () {
  ...
  root.classList.add('cap-native', 'cap-' + platform);
  ...
  ready(function () {
    ...
    setupOffline();
    setupPush();
    // hide the native splash once the page has painted
  ...
  function injectStyles() {
    ...
    css.textContent =
      // App Store reader model: no in-app purchase UI. Subscriptions/ebooks are bought on the web.
      ...
      // bottom tab bar
      '.cap-tabbar{position:fixed;left:0;right:0;bottom:0;z-index:9998;display:flex;' +
      ...
      'font:600 10.5px/1 Inter,system-ui,sans-serif;letter-spacing:.01em;-webkit-tap-highlight-color:transparent;}' +
  ...
  function registerToken(value) {
    ...
    if (!cfg.FUNCTIONS_BASE || !jwt) return; // not signed in yet — will register on a later page load
  ...
  function setupPush() {
    ...
    // small delay so the prompt doesn't fight first paint
    setTimeout(go, 1600);
  }
})();
