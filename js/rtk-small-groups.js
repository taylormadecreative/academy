...
export function createSmallGroups({ sb, copy, el, esc, key, isRoom, words, host, uid, rootId, getMeeting, myIdIn, hands, toast, confirmInline, onTick, closeSheet, facilitator }) {
  ...
  function startTick() {
    ...
    tick = setInterval(() => { if (onTick) onTick(); paintBoard(); if (Date.now() >= timer.endsAt + 1500) clearInterval(tick); }, 1000);
  }
  ...
  const setTimer = (minutes) => {
    ...
    const payload = () => ({ type: 'timer', endsAt: timer.endsAt ? new Date(timer.endsAt).toISOString() : null, minutes: timer.minutes });
    send(payload());
  ...
  async function askHelp() {
    ...
    if (error && error.code !== '23505') { toast('Could not ask for help — ' + error.message); return; }
    await hands.load();
  }
  ...
  function primary(container) {
    ...
    container.querySelector('.r2-back').addEventListener('click', async (ev) => { const b = ev.currentTarget; b.disabled = true; try { await goRoot(); } catch (e) { toast('Could not move you back — ' + (e.message || e)); b.disabled = false; } });
  }
  ...
  const roster = async () => {
    ...
    try { const { data } = await sb.rpc('ea_opil_roster_teams'); if (!Array.isArray(data)) return null; const map = new Map(); data.forEach(r => map.set(r.user_id, r.team_name)); return map; } catch (e) { return null; }
  };
  const many = copy.capFirst(words.many);
  const setupHTML = () => `<div class="r2-groups r2-board r2-board-setup">
      <p class="r2-fine" style="text-align:left;margin:0">${many} are split into rooms and moved on their own. You stay in the main room and visit any room from the board. Bring everyone back closes the rooms.</p>
      <div class="r2-split">
      ...
      <p class="r2-fine" style="text-align:left">${isRoom ? '' : 'By team puts each team in its own room, named after the team; anyone without a team shares an Open room. '}Everyone in a room sees the clock; when it hits zero nobody is moved until you press Bring everyone back.</p>
    </div>`;
  const cardHTML = (r, help) => {
    ...
    return `<div class="r2-room-card tone-${st.tone}" data-room="${esc(r.id)}">
      ...
      <div class="r2-room-actions">
        <button type="button" class="r2-mini r2-bring" data-visit="${esc(r.id)}"${me ? ' disabled' : ''}>${me ? 'You are here' : help ? 'Join room' : 'Peek in'}</button>
        <button type="button" class="r2-mini" data-msg="${esc(r.id)}">Message room</button>
      </div>
      <form class="r2-room-msg" data-room="${esc(r.id)}" hidden><input maxlength="200" placeholder="One line for this room…" autocomplete="off"><button type="submit" class="r2-mini r2-bring">Send</button></form>
    </div>`;
  };
  const boardHTML = (rooms) => {
    ...
    return `<div class="r2-groups r2-board">
      <div class="r2-board-head">
        <div><b>Small Groups in Progress</b><span>${many} are in their rooms. Visit any room or bring everyone back.</span></div>
        <form class="r2-note-all"><input maxlength="200" placeholder="Message all rooms…" autocomplete="off"><button type="submit" class="r2-mini r2-bring">Send to all</button></form>
      </div>
  ...
  async function refresh() {
    ...
    let rooms; try { rooms = await readRooms(); } catch (e) { board.innerHTML = `<div class="r2-empty">Could not read the rooms — ${esc(e.message || e)}</div>`; return; }
  ...
  function wire(root) {
    ...
    root.querySelectorAll('[data-visit]').forEach(b => b.addEventListener('click', async () => {
      ...
      catch (e) { toast('Could not move you — ' + (e.message || e)); b.disabled = false; }
    }));
  ...
  return { start, stop, left, primary, bindNote, board: boardPane, onHands: () => paintBoard(), inRoom };
}
