...
server.registerTool(
  "generate_academy_content",
  {
    ...
    inputSchema: {
      ...
      logo: z.boolean().optional().describe("Whether to stamp the academy logo. ON by default — it's placed like a designer would: small, tucked into the quietest corner of THAT image (varies per post), reversed to white on dark areas, never a center-bottom chip. Set false to omit the logo entirely for a post."),
  ...
  async ({ recipe, topic, notes, format, size, variants, engine, outfit, brands, logo, slides }) => {
    ...
    if (logo === false) args.push("--no-logo"); // logo is ON by default (smart placement)
